...
# local imports
from ..responses.models.base import ModelResponses


class BaseModel(ModelResponses):
    """Class that offers common data structure operations"""
    def __init__(self, dt_name):
        super().__init__(dt_name)
        self.dt_name = dt_name
        self.response = ""

    def insert_entries(self, *args):
        """Method that append entries to a given data structure"""
        new_entry = [row for row in args]
        if not self.dt_name:
            self.dt_name.extend(new_entry)
            self.response = self.creates_response()
        # common k, v lookup
        match = [row.items() & data.items() for row in args for data in self.dt_name]
        if match:
            exist_values = []
            for _, items in enumerate(match[0]):
                _, val = items
                exist_values.append(val)
            # _, v = next(iter(match[0]))
