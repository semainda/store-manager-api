...
def ownner_login(client, user_data):
    ...
    response = client.post(
        "api/v1/auth/login", data=json.dumps(user_data),
...
def test_get_specific_store_user(test_client, user_id, resp_code):
    ...
    response = test_client.get(
        "api/v1/users/" + str(user_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_get_specific_store_role(test_client, role_id, resp_code):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.get(
        "api/v1/roles/" + str(role_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_update_specific_store_role(test_client, role_name, role_id, resp_code, msg):
    ...
    response = test_client.put(
        "api/v1/roles/" + str(role_id), data=json.dumps(role_name), 
        headers={
            "content-type": "application/json",
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test__user_role_assignment(test_client, user_role, resp_code, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.post(
        "api/v1/users/roles", data=json.dumps(user_role), 
        headers={
            "content-type": "application/json",
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_get_specific_user_role(test_client, user_role_id, resp_code):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.get(
        "api/v1/users/roles/" + str(user_role_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_update_user_role(test_client, role_id, user_id, resp_code, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.put(
        "api/v1/users/roles/" + str(user_id), data=json.dumps(role_id),
        headers={
            "content-type": "application/json",
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_get_specific_category(test_client, cat_id, resp_code):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.get(
        "api/v1/categories/" + str(cat_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_update_specific_category(test_client, cat_name, cat_id, resp_code, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.put(
        "api/v1/categories/" + str(cat_id), data=json.dumps(cat_name), 
        headers={
            "content-type": "application/json",
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
def test_create_subcategories(test_client, sub_name, resp_code, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.post(
        "api/v1/subcategories", data=json.dumps(
            sub_name), headers={
                "content-type": "application/json",
                "Authorization": "Bearer {}".format(access_token)}
        )
...
def test_get_specific_subcategory(test_client, sub_id, resp_code):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.get(
        "api/v1/subcategories/" + str(sub_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
...
subcategories = [
    ...
    (dict(
        ...
        b"Key cat_id not found"),
    (dict(sub_cat_name="Drinks", cat_id=3), 404, 3, b"Categorie with key value '3' does not exists"),
    (dict(sub_cat_name="HPTouch", cat_id=1), 201, 1, b"Sub_Categorie with id '1' updated successful"),
...
def test_update_subcategories(test_client, sub_name, resp_code, sub_id, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.put(
        "api/v1/subcategories/" + str(sub_id), data=json.dumps(
            sub_name), headers={
                "content-type": "application/json",
                "Authorization": "Bearer {}".format(access_token)}
        )
...
products = [
    ...
    (dict(
        prod_name="HD Curved TV", price=450000,
        quantity=150, size=43, cat_id=6, sub_cat_id=1), 404, b"Categorie with key value '6' does not exists"),
     (dict(
        prod_name="HD Curved TV", price=450000,
        quantity=150, size=43, cat_id=1, sub_cat_id=5), 404, b"Sub_Categorie with key value '5' does not exists")
    ]
...
def test_owner_create_products(test_client, product, resp_code, msg):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.post(
        "api/v1/products", data=json.dumps(
            product), headers={
                "content-type": "application/json",
                "Authorization": "Bearer {}".format(access_token)}
        )
...
def test_get_specific_product(test_client, p_id, resp_code):
    access_token = ownner_login(test_client, owner_data)
    response = test_client.get(
        "api/v1/products/" + str(p_id), headers={
            "Authorization": "Bearer {}".format(access_token)})
    assert response.status_code == resp_code
